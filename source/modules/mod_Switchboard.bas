...
Else
    ...
    Do Until rst.EOF
        ...
        If Right(rst!Link_file_path, 6) <> ".accdb" Then
            MsgBox "Expected *.accdb file, found " & strSourceFileName & vbCrLf & _
                    "File NOT copied.", vbCritical, "Error creating data backup"
            GoTo Exit_Procedure
        Else
            strFilename = Left(rst!Link_file_path, Len(rst!Link_file_path) - 4) & _
                "_" & strBackupDate & ".accdb"
            ' Open the save file dialog and update to the actual name given by the user
            strCopyFileName = fxnSaveFile(strFilename, "Access (*.accb)", "*.accdb")
